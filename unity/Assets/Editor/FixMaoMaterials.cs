...
namespace Siro.EditorTools
{
    public static class FixMaoMaterials
    {
        ...
        public static void Fix()
        {
            ...
            if (normalMat == null)
            {
                Debug.LogError("❌ 找不到 UnlitBlendModeNormal.mat");
                return;
            }
            ...
            Debug.Log($"✓ 修正了 {fixedCount} 個 ArtMesh 材質");
            ...
            // 順便存 prefab asset（這樣下次重開材質也對）
            var prefabPath = "Assets/Live2D/Cubism/Samples/Models/Mao/Mao.prefab";
            var prefab = PrefabUtility.GetCorrespondingObjectFromSource(mao);
            if (prefab != null)
            {
                PrefabUtility.SaveAsPrefabAsset(mao, prefabPath);
                Debug.Log($"✓ 也存了 {prefabPath}");
            }
        }
