...
namespace Siro.EditorTools
{
    public static class MaoDiagnostic
    {
        ...
        public static void Diagnose()
        {
            ...
            // 自動修正 Mao 位置/Scale
            if (mao.transform.position != Vector3.zero ||
                mao.transform.localScale != Vector3.one)
            {
                Undo.RecordObject(mao.transform, "Reset Mao Transform");
                mao.transform.position = Vector3.zero;
                mao.transform.localScale = Vector3.one;
                mao.transform.rotation = Quaternion.identity;
                sb.AppendLine($"  ✓ 已自動歸零 (Position=0, Scale=1, Rotation=0)");
            }
