        ...
        public static void BuildChatUI()
        {
            ...
            if (!HasTMPEssentialResources())
            {
                ...
                if (import)
                {
                    ...
                    // Unity 6 把它放在 "Package Resources/" 資料夾下（不是 Runtime/）
                    ...
                    if (dirs.Length > 0)
                    {
                        // 兩種可能路徑都試
                        string[] candidates = {
                            Path.Combine(dirs[0], "Package Resources", "TMP Essential Resources.unitypackage"),
                            Path.Combine(dirs[0], "Runtime", "TMP Essential Resources.unitypackage"),
                        };
                        string essentials = null;
                        foreach (var c in candidates)
                        {
                            if (System.IO.File.Exists(c)) { essentials = c; break; }
                        }
                        if (essentials != null)
                        {
                            AssetDatabase.ImportPackage(essentials, false);
                            Debug.Log($"[SIRO UI Builder] 已 import TMP Essential Resources: {essentials}");
                        }
                        else
                        {
                            Debug.LogError(
            ...
            if (canvas == null)
            {
                ...
                scaler.matchWidthOrHeight = 0f;  // 0 = 跟寬度一致，1 = 跟高度一致
                canvasGO.AddComponent<GraphicRaycaster>();
                Undo.RegisterCreatedObjectUndo(canvasGO, "Create SIRO Canvas");
            }

            // 2. 找或建 EventSystem
            // 用 InputSystemUIInputModule（新 Input System）而非 StandaloneInputModule
            // 依 ProjectSettings.activeInputHandler 自動選
